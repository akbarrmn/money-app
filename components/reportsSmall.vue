<template>
  <div class="row-span-2 block lg:hidden border-2 border-[#B4B8B9]/30 rounded-2xl p-6 dark:bg-[#313234]">
        <div class="flex flex-col gap-3 h-full">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div v-for="i in getReports" :key="i.id" class="grid grid-cols-4 gap-2 text-sm lg:text-md">
                <p class="dark:text-white truncate">{{ i.transaction_name }}</p>
                <p class="col-span-2 text-gray-400">{{ useDateParse(JSON.parse(JSON.stringify(i.createdAt))) }}</p>
                <p v-bind:class="{ 'text-green-500': i.transaction_type, 'text-red-500': !i.transaction_type }">{{ useFormatCurrency(i.transaction_amount) }}</p>
            </div>
          </div>
        </div>
      </div>
